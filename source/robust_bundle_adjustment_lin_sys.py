...
from robust_estimation import robust_weight



def old_build_robust_linear_system_poses(XR, XL, Zr, kernel_threshold, num_poses, num_landmarks, pose_dim, landmark_dim):
    ...
    for measurement_num in range(Zr.shape[2]):
        ...
        chi_tot += chi
        # residual function
        residuals.append(np.linalg.norm(e))
    ...
    return H, b, chi_tot, num_inliers, residuals



def old_build_robust_linear_system_projections(XR, XL, Zp, associations, kernel_threshold, num_poses, num_landmarks, pose_dim, landmark_dim,
...
def build_robust_linear_system_poses(XR, XL, Zr, kernel_threshold, num_poses, num_landmarks, pose_dim, landmark_dim, robust_method, robust_param):
    system_size = pose_dim * num_poses + landmark_dim * num_landmarks
    H = np.zeros((system_size, system_size))
    b = np.zeros(system_size)
    chi_tot = 0
    ...
    for measurement_num in range(Zr.shape[2]):
        ...
        weight = robust_weight(residual, robust_method, robust_param)

        pose_i_matrix_index = pose_matrix_index(measurement_num, pose_dim, num_poses)
        pose_j_matrix_index = pose_matrix_index(measurement_num + 1, pose_dim, num_poses)

        # ===== H MATRIX =====
        H[pose_i_matrix_index:pose_i_matrix_index + pose_dim, pose_i_matrix_index:pose_i_matrix_index + pose_dim] += weight * Ji.T @ Omega @ Ji
        H[pose_i_matrix_index:pose_i_matrix_index + pose_dim, pose_j_matrix_index:pose_j_matrix_index + pose_dim] += weight * Ji.T @ Omega @ Jj
        H[pose_j_matrix_index:pose_j_matrix_index + pose_dim, pose_i_matrix_index:pose_i_matrix_index + pose_dim] += weight * Jj.T @ Omega @ Ji
        H[pose_j_matrix_index:pose_j_matrix_index + pose_dim, pose_j_matrix_index:pose_j_matrix_index + pose_dim] += weight * Jj.T @ Omega @ Jj

        # ===== b VECTOR =====
        b[pose_i_matrix_index:pose_i_matrix_index + pose_dim] += weight * Ji.T @ Omega @ e
        b[pose_j_matrix_index:pose_j_matrix_index + pose_dim] += weight * Jj.T @ Omega @ e
...
def build_robust_linear_system_projections(XR, XL, Zp, associations, kernel_threshold, num_poses, num_landmarks, pose_dim, landmark_dim,
                                    K, camera_transformation, z_near, z_far, image_rows, image_cols, robust_method, robust_param):
    ...
    for measurement_num in range(Zp.shape[1]):
        ...
        is_valid, e, Jr, Jl = projection_error_and_jacobian(Xr, Xl, z, K, camera_transformation, z_near, z_far, image_rows, image_cols)
        if not is_valid:
            residuals.append(0)  # placeholder for non-valid measurement
            continue
        ...
        # ===== H MATRIX =====
        H[pose_matrix_idx:pose_matrix_idx + pose_dim, pose_matrix_idx:pose_matrix_idx + pose_dim] += weight * Jr.T @ Jr
        H[pose_matrix_idx:pose_matrix_idx + pose_dim, landmark_matrix_idx:landmark_matrix_idx + landmark_dim] += weight * Jr.T @ Jl
        H[landmark_matrix_idx:landmark_matrix_idx + landmark_dim, landmark_matrix_idx:landmark_matrix_idx + landmark_dim] += weight * Jl.T @ Jl
        H[landmark_matrix_idx:landmark_matrix_idx + landmark_dim, pose_matrix_idx:pose_matrix_idx + pose_dim] += weight * Jl.T @ Jr

        # ===== b VECTOR =====
        b[pose_matrix_idx:pose_matrix_idx + pose_dim] += weight * Jr.T @ e
        b[landmark_matrix_idx:landmark_matrix_idx + landmark_dim] += weight * Jl.T @ e

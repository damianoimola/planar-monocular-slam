...
def build_robust_linear_system_poses(XR, XL, Zr, kernel_threshold, num_poses, num_landmarks, pose_dim, landmark_dim, robust_method, robust_param):
    ...
    for measurement_num in range(Zr.shape[2]):
        ...
        pose_i_matrix_index:pose_i_matrix_index + pose_dim] += weight * Ji.T @ Omega @ Ji

        H[pose_i_matrix_index:pose_i_matrix_index + pose_dim,
        pose_j_matrix_index:pose_j_matrix_index + pose_dim] += weight * Ji.T @ Omega @ Jj

        H[pose_j_matrix_index:pose_j_matrix_index + pose_dim,
        pose_i_matrix_index:pose_i_matrix_index + pose_dim] += weight * Jj.T @ Omega @ Ji

        H[pose_j_matrix_index:pose_j_matrix_index + pose_dim,
        pose_j_matrix_index:pose_j_matrix_index + pose_dim] += weight * Jj.T @ Omega @ Jj
...
def build_robust_linear_system_projections(XR, XL, Zp, associations, kernel_threshold, num_poses, num_landmarks, pose_dim, landmark_dim,
    ...
    for measurement_num in range(Zp.shape[1]):
        ...
        pose_matrix_idx:pose_matrix_idx + pose_dim] += weight * Jr.T @ Jr

        H[pose_matrix_idx:pose_matrix_idx + pose_dim,
        landmark_matrix_idx:landmark_matrix_idx + landmark_dim] += weight * Jr.T @ Jl

        H[landmark_matrix_idx:landmark_matrix_idx + landmark_dim,
        landmark_matrix_idx:landmark_matrix_idx + landmark_dim] += weight * Jl.T @ Jl

        H[landmark_matrix_idx:landmark_matrix_idx + landmark_dim,
        pose_matrix_idx:pose_matrix_idx + pose_dim] += weight * Jl.T @ Jr
